...
export function readInputLines(files) {
  const lines = [];
  const fileNames = [];
  const fileStats = [];
  for (const file of files) {
    ...
    for (const [index, line] of text.split(/\r?\n/).entries()) {
      ...
      try {
        ...
        normalized = obj.log || obj.logstr || obj.log_str || obj.str_error_msg || obj.M || JSON.stringify(obj);
      } catch {
        normalized = trimmed;
      }
      lines.push({
...
function computeCacheKey({ files, apiDir, options }) {
  ...
  hash.update(JSON.stringify({ version: 4, options }));
...
if (isCli) main().catch(error => fail(error.message, 2));
