...
const TUI_HINT_RE = /Tuikit|TUIRoom|TUILive|TUICall|RTCRoom|RoomEngine|LiveKit|RoomKit|CallKit/i;
...
function toLineItems(lines) {
  return lines
    .map((line, index) => {
      const physicalLine = Number.isInteger(line?.line) ? line.line : index + 1;
      return {
        text: lineText(line),
        line: physicalLine,
        endLine: Number.isInteger(line?.endLine) ? line.endLine : physicalLine,
...
export function groupLogEntries(lines, logType) {
  ...
  for (const item of items) {
    if (isStart(item) || current == null || item.source !== current.source) {
...
export function renderTimelineMarkdown(timeline) {
  ...
  for (const e of timeline.events) {
    ...
    const source = safeMarkdownTableCell(e.source || '', { maxChars: 120 });
    lines.push(`| ${time} | ${source} | L${e.line || e.index || ''} | ${level} | ${rule} | ${desc} |`);
  }

  if (timeline.events.length > 0) {
    const evidence = timeline.events
      .map(e => safeEvidenceLine(e.line || e.index || '', e.log || e.text || '', {
        maxChars: 500,
        source: e.source || '',
      }))
      .join('\n');
